...
# ─── OdoCrypt consensus gate ────────────────────────────────────────────────
# The full libmultihash link has historically shipped a non-consensus OdoCrypt
# (a stale/mis-linked build got deployed and rejected 100% of DGB odo shares for
# weeks). build_test_odocrypt.sh only tests odocrypt.cpp in isolation and cannot
# catch that. Here we dlopen the ACTUAL produced libmultihash.so and check its
# exported odocrypt_export against a fixed DigiByte-consensus vector. Fail the
# whole native build if it mismatches, so a bad OdoCrypt can never ship.
echo "Verifying OdoCrypt consensus of built libmultihash.so ..."
cc ../Native/libmultihash/verify_odocrypt_so.c -ldl -o /tmp/verify_odocrypt_so
if ! /tmp/verify_odocrypt_so "$OutDir/libmultihash.so"; then
    echo "FATAL: built libmultihash.so fails OdoCrypt consensus — aborting native build" >&2
    exit 1
fi
